...
public class GetAppHomePageAsync extends AsyncTask<HomePageInputModel, Void, Void> {
    ...
    protected Void doInBackground(HomePageInputModel... params) {
        ...
        try {
           ...
           parameters.put(HeaderConstants.AUTH_TOKEN, this.homePageInputModel.getAuthToken());
           parameters.put(HeaderConstants.LANG_CODE, this.homePageInputModel.getLang_code());
           parameters.put(HeaderConstants.USER_ID, this.homePageInputModel.getUserId());

            // Execute HTTP Post Request
            try {
                responseStr = Utils.requester.addHeaderParams(parameters, APIUrlConstant.getHomepageUrl());
            } catch (Exception e) {
                status = 0;
                message = "Error";
                e.printStackTrace();
